...
AddEventHandler("OnPlayerConnectFull", function(event)
	...
	db:Query("select * from "..config:Fetch("admins.tablenames.comms").." where (player_steamid = '"..tostring(player:GetSteamID()).."') and serverid = "..config:Fetch("admins.serverid").." and (expiretime = 0 OR expiretime - UNIX_TIMESTAMP() > 0) order by id limit 1", function(err, result)
...
AddEventHandler("OnClientChat", function(event, playerid, text, teamonly)
	...
	if teamonly and text:sub(1,1) == "@" then
		...
		if not exports["admins"]:HasFlags(playerid, "j") then
			player:SendMsg(MessageType.Chat, FetchTranslation("admins.chat.to_admins"):gsub("{NAME}", player:CBasePlayerController().PlayerName):gsub("{TEXT}", sendText))
		end

		local tosendmsg = FetchTranslation("admins.chat.admin_chat"):gsub("{NAME}", player:CBasePlayerController().PlayerName):gsub("{TEXT}", sendText)
	...
	if player:GetVar("player_gagged") then
		...
		if duration > 0 then
			ReplyToCommand(playerid, config:Fetch("admins.prefix"), FetchTranslation("admins.gag.try"):gsub("{TIME}", ComputePrettyTime(duration)))
			event:SetReturn(false)
